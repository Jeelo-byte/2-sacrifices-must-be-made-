...
var dash_drain_timer: float = 0.0
var dash_drain_rate: float = 1.0

var machine_gun_timer: float = 0.0
var machine_gun_rate: float = 0.1
var machine_gun_drain_timer: float = 0.0
var machine_gun_drain_rate: float = 0.5
...
func _physics_process(delta):
	...
	if current_health != 0:
		...
		if Input.is_action_just_pressed("LMB"):
			fire()
		elif Input.is_action_pressed("LMB") and Input.is_action_pressed("machine_gun"):
			machine_gun_timer += delta
			machine_gun_drain_timer += delta
			if machine_gun_timer >= machine_gun_rate:
				fire()
				machine_gun_timer = 0.0
			if machine_gun_drain_timer >= machine_gun_drain_rate:
				current_health = max(current_health - 1, 0)
				health_changed.emit(current_health, max_health)
				machine_gun_drain_timer = 0.0
		else:
			machine_gun_timer = 0.0
			machine_gun_drain_timer = 0.0
		...
		if is_dashing and direction != Vector2.ZERO:
			dash_drain_timer += delta
			if dash_drain_timer >= dash_drain_rate:
				current_health = max(current_health - 1, 0)
				health_changed.emit(current_health, max_health)
				dash_drain_timer = 0.0
		else:
			dash_drain_timer = 0.0
